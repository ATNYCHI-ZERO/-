...
FALLBACK_FILENAME = "Complete_Riemann_Hypothesis_Validation_Paper.txt"
...
def build_fallback_text(output_path: Path) -> Path:
    """Write the paper content to a UTF-8 text file when ReportLab is missing."""

    output_path.write_text(FULL_TEXT.strip() + "\n", encoding="utf-8")
    return output_path
...
def main() -> None:
    if REPORTLAB_AVAILABLE:
        ...
        output_path = Path(__file__).resolve().with_name(FALLBACK_FILENAME)
        built_path = build_fallback_text(output_path)
        print(
            "ReportLab is unavailable; wrote fallback text representation to:"
            f" {built_path}"
        )
